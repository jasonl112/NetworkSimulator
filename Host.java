...
public abstract class Host {
    ...
    /**
     * This is called after a Timer event expires, and enables you to write code to
     * do something upon timer
    ...
    /**
     * This is called after a Timer event is cancelled, and enables you to write
     * code to do something upon timer
     * cancellation.
     *
    ...
    /**
     * Assign this host's address and provide a reference to the FutureEventList
    ...
    /**
     * Return the address of this Host object. This is the address assigned using
     * setHostParameters.
     *
    ...
    /**
     * Adds a new neighboring host to this host. A neighbor is a host directly
     * connected to this host, with the
     * specified distance between them. The further apart two hosts are, the longer
     * it takes for a message to traverse
    ...
    /**
     * Sends a Message object to the neighbor specified by the Message object's
     * destination address.
     *
    ...
    protected void sendToNeighbor(Message msg) {
        ...
        if (neighbor == null) {
            throw new EventException(
                    (short) 0, "sendToNeighbor: destination address of message is unknown: " + msg.getDestAddress());
        }
    ...
    /**
     * Create a new Timer event, managed by this class. What you receive from this
     * method is the event id of the timer,
     * not the Timer object. When a timer expires or is cancelled, a callback method
     * will be invoked (see below) with
     * the timer's event id passed to it, so you'll know which timer expired or was
     * cancelled.
     *
    ...
    protected void cancelTimer(int timerId) {
        ...
        if (timer == null) {
            throw new EventException((short) 0,
                    "cancelTimer: attempted to cancel Timer event which does not exist: " + timerId);
        }
    ...
    /**
     * Called when a Timer event's cancel method is invoked. Performs clean-up, then
     * calls the timerCancelled method.
     *
    ...
    protected void onTimerCancellation(Timer timer) {
        ...
        if (tObj == null) {
            throw new EventException(
                    (short) 0,
                    "onTimerCancellation: attempted to remove Timer event which does not exist: " + timer.getId());
        }
        this.timerCancelled(timer.getId());
    }

    /**
     * Called when a Timer event's handle method is invoked. Performs clean-up, then
     * calls the timerExpired method.
     * <br>
     * Note that when a Timer event expires, it is removed. In other words, timers
     * are not recurring (they only expire
     * once). If you want a recurring timer, you'll need to create a new timer each
    ...
    protected void onTimerExpiry(Timer timer) {
        ...
        if (tObj == null) {
            throw new EventException(
                    (short) 0, "onTimerExpiry: attempted to remove Timer event which does not exist: " + timer.getId());
        }
